        ...
        <div className="grid gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Fonctionnalités accessibles avec votre rôle {getRoleName(user?.role)}
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {user?.role === 'SUPER_ADMIN' && (
                  <>
                    <div className="p-4 border rounded-lg hover:bg-gray-50">
                      ...
                      <h3 className="font-medium mb-1">Gestion des entreprises</h3>
                      <p className="text-sm text-gray-600">Créer et gérer les entreprises</p>
                    </div>
                    <div className="p-4 border rounded-lg hover:bg-gray-50">
                      <User className="h-8 w-8 text-green-600 mb-2" />
                      <h3 className="font-medium mb-1">Gestion des utilisateurs</h3>
                      <p className="text-sm text-gray-600">Administrer tous les utilisateurs</p>
                    </div>
                  </>
                )}
                ...
                {(user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN') && (
                  <>
                    <div className="p-4 border rounded-lg hover:bg-gray-50">
                      ...
                      <p className="text-sm text-gray-600">Ajouter et modifier les employés</p>
                    </div>
                    <div className="p-4 border rounded-lg hover:bg-gray-50">
                      <CreditCard className="h-8 w-8 text-orange-600 mb-2" />
                      <h3 className="font-medium mb-1">Cycles de paie</h3>
                      <p className="text-sm text-gray-600">Créer et approuver les paies</p>
                    </div>
                  </>
                )}

                {(user?.role === 'CASHIER' || user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN') && (
                  <div className="p-4 border rounded-lg hover:bg-gray-50">
                    <CreditCard className="h-8 w-8 text-red-600 mb-2" />
                    <h3 className="font-medium mb-1">Paiements</h3>
                    <p className="text-sm text-gray-600">Effectuer les paiements de salaires</p>
                  </div>
                )}
              </div>
            </CardContent>
